...
enum BLEProtocol {
    ...
    ///
    /// ⚠️ 实机复核点 — `crc32` 语义（方案 A）：调用方 `BoardSession.sendImage` 传入的是
    /// **Payload 层校验** `crc32c(payload)`（与 `ctnData` 包头一致），非 `crc32c(ctn)`。
    /// 若实机发图被板子拒绝，改为传 `crc32c(ctn)`（CtnData 层校验，方案 B）并更新此处注释。
