...
final class BoardSession: ObservableObject {
    ...
    func sendImage(width: Int, height: Int, rgb: [UInt8]) async throws {
        ...
        // ⚠️ 实机复核点 — StartStream 的 CRC 语义（P0-5 / 方案 A）
        // 此处的 crc32 传的是 **Payload 层校验**（= `ctnData` 包头里写入的 `crc32c(payload)`），
        // 使 StartStream 与 CtnData 包头自洽（方案 A）。
        // 若实机发图被板子拒绝（StartStream 校验不通过），回退为 `crc32c(ctn)`（CtnData 层校验，
        // 方案 B），并同步修改 `BLEProtocol.startStream` 的文档注释。
        let start = BLEProtocol.startStream(crc32: BLEProtocol.crc32c(payload), totalLength: ctn.count)
    ...
    /// 连接/握手后把 UI 状态同步到实物：开显示 + 设一次亮度（P1-3）。
    ///
    /// 仅当 `central.linkState == .connected` 时执行，避免对未连接/已断开设备发指令。
    /// 依次下发 `displayEnable` 与 `dimming`，保证 App 内开关/亮度与灯板实物一致。
    func syncDisplayState(on: Bool, brightnessPercent: Int) async {
        guard central.linkState == .connected else { return }
        await setDisplay(on)
        await setBrightness(level: level(forBrightnessPercent: brightnessPercent))
    }
}
