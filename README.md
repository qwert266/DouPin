...
| 图纸库 | 5 Tab 信息架构：首页 / 图纸 / 库存 / 拼豆板 / 我的。「图纸」页分「全部 / 文件夹 / 标签 / 模板」四段：搜索、状态筛选（待拼 / 拼制中 / 已完成）、**文件夹归档**、**标签过滤**、删除、进度百分比 |
| 库存管理 | 一个色号一条记录：逐条添加 / 批量文本导入（空格、半角逗号、全角逗号、制表符分隔均可）/ 数量编辑 / 缺色阈值预警 / 色系筛选 / 色号模糊搜索；**消耗预估**按图纸用色与库存对比，输出「需 / 有 / 缺」明细与可复制的缺色清单 |
...
| 智能拼豆板（BLE） | 扫描 / 连接 Wofan 拼豆板（A950 服务），自动握手；发完整预览图（已拼格暗显）；**行引导模式**：点亮当前行 + 相邻行微亮，可按颜色过滤，「本行拼完 → 下一行」自动打卡并前进 |
...
└── Sources/
    ├── App/       # 入口、AppState（全局共享 BoardSession）、5 Tab 容器、首页
    ...
    └── Views/     # 转换、编辑器、图纸库、文件夹、标签、库存、模板库、数据中心、合并图纸、拼豆板、作品详情、我的
```

> 工程共 36 个 Swift 源文件。`project.pbxproj` 为手写 plist 格式，新增文件需同步 4 处引用
