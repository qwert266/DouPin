...
| 设置 | 拼豆板规格（PD25/52/78/104，52钉≈14cm、78钉≈21cm、104钉≈28cm）、**色板档位**（295/291/264/221/144/72 或「仅我的库存色」，转图只用你买得到的色）、自动连接开关、默认亮度、主题、语言、触觉反馈、**检测更新**（对比仓库最新提交）、存储清理 |
...
| 颜色算法 | **CIEDE2000（ΔE00）感知色差**（经 Sharma 标准测试向量校验）+ **灰世界自动白平衡**（去室内暖光/冷光偏色）+ **块内抗混色采样**（亮度截尾均值，抗边缘与高光污染）+ 匹配采用「彩度加权粗筛前三 → ΔE00 精筛」保证精度同时可控耗时 + **限色收敛按「频次 × ΔE00」最小视觉损失合并** |
