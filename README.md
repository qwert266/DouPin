...
| 数据中心 | 对标 AI豆仓「记录与数据统计」：创作总览、**补豆清单**（全部图纸需求 vs 库存，一键复制）、**消耗排行 Top 15**（哪些色号用得最快）、**库存色系分布**、最近完成作品 |
| 作品集 | 对标 AI豆仓「作品集」：已完成作品归档（成品照优先，无照片用图纸网格）、筛选（全部/有成品照/本月完成）、大图查看 + **一键生成竖版作品分享卡**（成品照/图纸 + 尺寸豆量 + 色号 Top6 + 落款）走系统分享 |
| 合并图纸 | 对标 PIXDOU「多图合并排版」：选 2～9 张图纸按横向并排 / 纵向堆叠 / 两列网格合并成一张大图，间隔空白可调，实时预览，生成后可继续编辑与拆板 |
| 模板库 | **61 个原创像素模板**，按 **10 个套装**分组（像素小动物 / 表情包 / 自然风景 / 节日 / 食物甜点 / 字母数字 / 爱心系 / 植物花语 / 太空星球 / 日常物件），预览用色清单，一键开始拼 |
| 图纸库 | 4 Tab 信息架构：首页 / 图纸 / 库存 / 我的（「拼豆板」Tab 已并入全局连接胶囊，见下）。「图纸」页分「全部 / 文件夹 / 标签 / 模板」四段：搜索、状态筛选（待拼 / 拼制中 / 已完成）、**文件夹归档**、**标签过滤**、删除、进度百分比 |
| 库存管理 | **多豆仓分区**（对标 AI豆仓）：按品牌/用途建多个仓独立管理，仓间一键切换、重命名、解散（库存归回默认仓不丢数据）；一个色号一条记录
| 拆板与尺寸分离 | 大图纸按板尺寸切分为多块（支持重叠列，末块自动截断），生成子图纸并记录归属；适合 29×29 标准板拼大幅作品 |
| 作品详情 | 图纸查看 + 编辑 + 导出图片（网格 / 色号标注 / 图例）+ **导出 PDF 打印稿**（A4 分页 + 页码 + 末页图例）+ 复制豆子清单 |
| 照片转图纸增强 | 转换后保留**原图**，编辑器内可**左右分屏对比**原图与图纸；**合并相近色**（加权欧氏距离给出合并建议）；**替换配色**（整色号一键换色） |
| 小红书链接提取 | 粘贴小红书笔记链接，三级流水线提取笔记内图片，8 秒超时后降级为「保存到相册再导入」引导 |
| 完整进度打卡 | ① 逐行打卡（拼完一行点一下，可撤销）② 逐格打卡（点格子标记已拼）③ 分色打卡（整色一次标记）④ 成品照片存档；进度持久化，断点续拼 |
| 智能拼豆板（BLE） | **各页右上角「连接」胶囊**：点一下自动搜索并连接 PIXDOU/iLEDColor/Wofan 板子（A950 服务），自动握手；发完整预览图（已拼格暗显）；**行引导模式**：点亮当前行 + 相邻行微亮，可按颜色过滤，「本行拼完 → 下一行」自动打卡并前进 |
...
└── Sources/
    ├── Assets.xcassets/   # App 图标（AppIcon 1024×1024）
    ├── App/       # 入口、AppState（全局共享 BoardSession）、4 Tab 容器、首页
    ├── Core/      # Pattern / PatternFolder / BeadStock 模型、295 色板、像素转换、图纸渲染、61 个模板
    ├── BLE/       # A950 协议帧、CoreBluetooth、拼豆板会话（握手/发图/行引导）
    ├── Services/  # 拆板、合并相近色、消耗预估、库存文本解析、PDF 导出、小红书提取
    └── Views/     # 转换、编辑器、图纸库、文件夹、标签、库存、模板库、数据中心、作品集、合并图纸、拼豆板、作品详情、我的
```

> 工程共 37 个 Swift 源文件。`project.pbxproj` 为手写 plist 格式，新增文件需同步 4 处引用
> （`PBXFileReference` / `PBXBuildFile` / 所属 `PBXGroup` / `PBXSourcesBuildPhase`），
> 并保持「文件引用数 == 构建文件数 == Sources 构建阶段条目数」三者相等。
